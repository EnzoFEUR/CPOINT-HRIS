        ...
        const calculatePayroll = async () => {
            if (formData.employee_id && formData.period_start && formData.period_end) {
                ...
                try {
                    ...
                    const monthlyBase = parseFloat(
                        selectedEmployee?.monthly_salary ||
                        selectedEmployee?.salary ||
                        (selectedEmployee?.daily_rate ? selectedEmployee.daily_rate * doleDivisor : employeeRate)
                    );
                        ...
                        {periodDaysCount > 0 && (
                            <div className="flex flex-wrap items-center justify-between gap-2 bg-blue-50/70 border border-blue-100 px-4 py-2.5 rounded-xl text-xs text-blue-900 font-medium">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <span>
                                        {formatReadableDate(formData.period_start)} &rrArr; {formatReadableDate(formData.period_end)}
                                    </span>
                                </div>
                            ...
                            <div className="overflow-y-auto p-3 space-y-2 divide-y divide-slate-100">
                                {filteredEmployees.length === 0 ? (
                                    ...
                                    filteredEmployees.map((emp) => {
                                        ...
                                        const rate = parseFloat(emp.piece_rate || emp.rate_per_piece || emp.salary || emp.monthly_salary || emp.daily_rate || 0);
